...
class File_manifest {
	...
	public static function merge($manifest_filename, $header_format, $footer_format) {
		...
		foreach($manifest_files as $filepath) {
			...
			$filenane = relpath($filepath, $parent_dir_strlen);
	...
	public static function gen($manifest_filename, $echo_format, $glue="\n") {
		...
		$parent_dir_split = preg_split('/[\\/\\\\]/', $info['parent_dir']);
		...
		foreach($manifest_files as $filepath) {
			...
			$target_dir_split = preg_split('/[\\/\\\\]/', $filepath);
			foreach($parent_dir_split as $din => $dir) {
				if($dir == $target_dir_split[0] && sizeof($target_dir_split)!=0) {
					array_shift($target_dir_split);
				}
				else {
					$upal = true;
					print_r($target_dir_split);
					print_r(array_slice($parent_dir_split, $din));
					break;
				}
			}
		if(isset($upal)) {
			$filepath = str_repeat("../", sizeof($parent_dir_split) - $din)
				.implode('/',$target_dir_split);
		}
		else {
			$filepath = substr($filepath, $parent_dir_strlen);
		}
			
			// show the start of a file
			$echo []= preg_replace('/%PATH%/', $filepath, $echo_format);
		}
