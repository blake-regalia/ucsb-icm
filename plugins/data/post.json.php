...
foreach($requests as $req) {
	if(!preg_match('/^([a-zA-Z_.-][a-zA-Z0-9_.-]*)\\/([^\\/]+)\\.(json|sql|html)(!?)$/', $req, $matches)) {
		$json []= '{"error":"could not digest string"}';
		continue;
	}
	...
	if(!preg_match('/^([a-z_-][a-z_0-9\\.-]*)(.*)$/i', $query, $uri)) {
		$json []= '{"error":"string does not match"}';
		continue;
	}
	...
	$fname = $db_name.'/'.$query.'.json';
	
	// if this query has already been cached, return the file
	if(!$do_not_cache && file_exists($fname)) {
		$json []= file_get_contents($fname);
	}	
}

json_headers();
echo '{"responses":['.implode(',',$json).']}';
exit;
